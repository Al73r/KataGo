...
## Windows on ARM64 (Snapdragon) — QNN / Hexagon NPU backend
   ...
   * Scope notes:
      * Non-metadata models only (the ONNX emitter does not emit SGF-metadata inputs). Convnets are the primary target; transformer nets may fall back to CPU on the HTP in FP16.
